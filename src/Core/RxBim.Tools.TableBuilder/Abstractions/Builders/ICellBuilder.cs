    ...
    where TItem : TableItemBase
{
    ...
    ICellBuilder<TItem> MergeNext(int count = 1, Action<ICellBuilder<TItem>, ICellBuilder<TItem>>? action = null);
    ...
    /// <param name="action">Delegate, applied to the cells to be merged.</param>
    /// <returns>Returns the <see cref="ICellBuilder{TItem}"/> of the original cell.</returns>
    ICellBuilder<TItem> MergeDown(int count = 1, Action<ICellBuilder<TItem>, ICellBuilder<TItem>>? action = null);
    ...
    ICellBuilder<TItem> MergeLeft(int count = 1);

    /// <summary>
    /// Sets format for the cell.
    /// </summary>
    /// <param name="format">Format value.</param>
    ICellBuilder<TItem> SetFormat(CellFormatStyle format);

    /// <summary>
    /// Sets format for the cell.
    /// </summary>
    /// <param name="action">Format building action.</param>
    ICellBuilder<TItem> SetFormat(Action<ICellFormatStyleBuilder> action);
}
